  ...
  public void testComputeEmbedding() throws IOException {
    ...
    try (Directory directory = newDirectory()) {
      ...
      try (var dict = new KnnVectorDict(directory, "dict")) {
        var demoEmbeddings = new DemoEmbeddings(dict);
        ...
        var realVector = demoEmbeddings.computeEmbedding("the real fact");
        assertEquals(50, realVector.length);

        var the = getTermVector(dict, "the");
        assertArrayEquals(new float[50], getTermVector(dict, "real"), 0);
        var fact = getTermVector(dict, "fact");
